...
public abstract class ZuulFilter implements IZuulFilter, Comparable<ZuulFilter> {
    ...
    public ZuulFilterResult runFilter() {
        ZuulFilterResult zfr = new ZuulFilterResult();

        if (!filterDisabled.get()) {
            if (shouldFilter()) {
                try {
                    Object res = run();
                    zfr.setStatus(ExecutionStatus.SUCCESS);
                    zfr.setResult(res);
                } catch (Throwable t) {
                    zfr.setException(t);
                    zfr.setStatus(ExecutionStatus.FAILED);
                }
            } else {
                zfr.setStatus(ExecutionStatus.SKIPPED);
            }
        }
